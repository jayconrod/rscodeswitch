...
pub trait LuaRuntime {
    fn compile(&self, path: &Path, data: &[u8]) -> Result<package::Package, ErrorList>;
    fn convert_number(&self, s: &str) -> LuaConvertedNumber;
}

pub enum LuaConvertedNumber {
    Malformed,
    Int(i64),
    Float(f64),
}

pub struct LuaRuntimeUnimplemented {}

impl LuaRuntime for LuaRuntimeUnimplemented {
    fn compile(&self, _: &Path, _: &[u8]) -> Result<package::Package, ErrorList> {
        unimplemented!();
    }

    fn convert_number(&self, _: &str) -> LuaConvertedNumber {
        unimplemented!();
    }
}
